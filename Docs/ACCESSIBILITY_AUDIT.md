...
| Edit | Implemented in source | Pattern name, meter, global subdivision, visual rhythm-grid beat groups, per-beat subdivision selectors, and explicit beat/subdivision accent/mute controls are labeled. Grid steps announce the step number and current accent level. |
...
- Tapping a grid step cycles its accent state; the context menu exposes explicit Strong, Normal, Ghost, and Mute choices.
