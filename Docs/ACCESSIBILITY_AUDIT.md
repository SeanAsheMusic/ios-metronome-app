...
| Edit | Implemented in source | Pattern name, meter, global subdivision, visual rhythm-grid beat groups, per-beat subdivision selectors, an Accent/Hit/Soft/Rest paint rail, and explicit step-pad controls are labeled. Grid steps announce the step number and current accent level. |
...
- The rhythm-grid paint rail exposes Accent, Hit, Soft, and Rest states; tapping a large step pad applies the selected state.
