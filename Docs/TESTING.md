...
The script verifies the privacy manifest and local-first policy, runs `swift test`, verifies `SoundLibrary/RenderedVerification/SHA256SUMS`, runs the shared `RhythmModel`, `AudioEngine`, `Persistence`, and `MetronomeUITests` Xcode test schemes, rejects Xcode warning output, builds the `Metronome` app for an iPhone simulator and a generic iOS device with code signing disabled, creates an unsigned generic iOS archive containing `Click Track.app`, verifies archived app metadata and privacy manifest contents, installs the simulator build, launches it, captures Onboarding, Play, Edit rhythm-grid, Patterns, Setlist, Settings, and Stage Pulse smoke-test screenshots, rejects tiny or incomplete captures, and terminates it. The UI smoke tests launch Click Track with an in-memory default library, assert the first-launch setup can complete into Play, assert the Play screen and redesigned performance controls are visible, tap through all five primary navigation surfaces, verify the Edit rhythm grid can launch directly into the editor and paint a hit and rest step, verify Patterns exposes template and saved-pattern controls, verify Setlist exposes song-form controls, verify Settings exposes click sound, timing-validation, MIDI, Beat Coach, and local data controls, verify Stage Pulse opens and closes from the Play surface, verify Stage Pulse can launch directly for capture, verify playback updates the beat visualizer from a scheduled audio-engine event, and smoke-test core Play controls at the largest accessibility text size. It defaults to `platform=iOS Simulator,name=iPhone 17`; pass another destination string as the first argument when needed.
...
xcodebuild archive -project Metronome.xcodeproj -scheme Metronome -destination 'generic/platform=iOS' CODE_SIGNING_ALLOWED=NO SKIP_INSTALL=NO -archivePath /tmp/ClickTrackLocalValidation/ClickTrack.xcarchive
xcrun simctl install booted "/tmp/ClickTrackLocalValidation/Metronome/Build/Products/Debug-iphonesimulator/Click Track.app"
