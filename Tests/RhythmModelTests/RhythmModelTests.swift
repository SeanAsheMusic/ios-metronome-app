...
final class RhythmModelTests: XCTestCase {
    ...
    func testClaveModeTemplatesCoverTraditionalAndOddFeels() {
        XCTAssertEqual(ClaveModeTemplate.allCases.count, 17)
        XCTAssertEqual(Pattern.claveMode(.bembeBell68).beats.filter { $0.soundRole == .beat }.map(\.index), [2, 3, 4, 5, 6, 7, 8, 10])
        XCTAssertEqual(Pattern.claveMode(.afroCubanBell68, mixerPreset: .claveOnly).beats.filter { $0.soundRole != .muted }.map(\.index), [0, 3, 6, 8, 10])
        XCTAssertEqual(Pattern.claveMode(.sevenEightTwoThreeTwo, mixerPreset: .claveOnly).meter.grouping, [2, 3, 2])
        XCTAssertEqual(Pattern.claveMode(.nineEightThreeThreeThree, mixerPreset: .claveOnly).beats.filter { $0.soundRole != .muted }.map(\.index), [0, 3, 6, 9, 12, 15])
        ...
        XCTAssertEqual(Pattern.claveMode(.twelveEightBell, mixerPreset: .claveOnly).beats.filter { $0.soundRole != .muted }.map(\.index), [0, 3, 6, 10, 12, 15, 18, 22])
    }
